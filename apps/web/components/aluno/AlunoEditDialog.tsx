...
export type EditAluno = {
  ...
  dataNasc?: string | null; // ISO string
  cpf?: string | null;
  email?: string | null;
...
export function AlunoEditDialog({ open, onOpenChange, aluno, onSaved }: Props) {
  ...
  const [dataNasc, setDataNasc] = React.useState(""); // yyyy-mm-dd
  ...
  // Modalidade e nível removidos deste modal (apenas no Wizard)
  ...
  // Modalidade/Nível removidos do modal de edição
  ...
  // Preferências/consentimentos/tamanhos removidos do modal de edição
      setStatus(aluno.status || "ATIVO");
      setCodigoInterno((aluno.codigoInterno || "") as string);
  // Tags removidas

      // Responsável principal (se disponível)
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Endereço
      const end: Partial<{ cep: string; logradouro: string; numero: string; complemento: string; bairro: string; cidade: string; uf: string; }> = {};
  ...
  // Preferências/financeiro/consentimentos/tamanhos removidos do modal de edição
  ...
  const re: Partial<{ cep: string; logradouro: string; numero: string; complemento: string; bairro: string; cidade: string; uf: string; }> = {};
          ...
          <form onSubmit={onSubmit} className="space-y-6">
            ...
            <div className="rounded-md border p-3 space-y-3">
              ...
              <div className="space-y-1 md:col-span-3">
                ...
                <textarea value={observacao} onChange={(e) => setObservacao(e.target.value)} rows={3} className="w-full border rounded px-2 py-1 text-sm" placeholder="Notas gerais" />
              </div>
              <div className="space-y-1">
                <label className="text-xs text-slate-600">Gênero</label>
                <Select value={genero || undefined} onValueChange={(v) => setGenero(v as EditAluno["genero"])}>
                  <SelectTrigger>
              ...
              <div className="space-y-1 md:col-span-3">
                ...
                <textarea value={alergias} onChange={(e) => setAlergias(e.target.value)} rows={2} className="w-full border rounded px-2 py-1 text-sm" />
              </div>
              <div className="space-y-1 md:col-span-3">
                <label className="text-xs text-slate-600">Restrições médicas</label>
                <textarea value={restricoes} onChange={(e) => setRestricoes(e.target.value)} rows={2} className="w-full border rounded px-2 py-1 text-sm" />
              </div>
              <div className="space-y-1">
                <label className="text-xs text-slate-600">Contato emergência - nome</label>
                <Input value={contatoNome} onChange={(e) => setContatoNome(e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs text-slate-600">Contato emergência - telefone</label>
                <IMaskInput
                  mask={["(00) 0000-0000", "(00) 00000-0000"]}
            ...
            {/* Preferências/financeiro/consentimentos removidos do modal de edição */}
            ...
            <div className="rounded-md border p-3 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs text-slate-600">E-mail</label>
                  <Input type="email" value={respEmail} onChange={(e) => setRespEmail(e.target.value)} placeholder="email@exemplo.com" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-600">Telefone</label>
                  <IMaskInput
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                ...
                <div className="space-y-1 md:col-span-3">
                  <label className="text-xs text-slate-600">Logradouro</label>
                  <Input value={respLogradouro} onChange={(e) => setRespLogradouro(e.target.value)} />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-600">Número</label>
                  <Input value={respNumero} onChange={(e) => setRespNumero(e.target.value)} />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-600">Complemento</label>
                  <Input value={respComplemento} onChange={(e) => setRespComplemento(e.target.value)} placeholder="Opcional" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-600">Bairro</label>
                  <Input value={respBairro} onChange={(e) => setRespBairro(e.target.value)} />
                </div>
                <div className="space-y-1 md:col-span-3">
                  <label className="text-xs text-slate-600">Cidade</label>
                  <Input value={respCidade} onChange={(e) => setRespCidade(e.target.value)} />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-600">UF</label>
                  <Input value={respUf} onChange={(e) => setRespUf(e.target.value.toUpperCase().slice(0,2))} placeholder="UF" maxLength={2} />
                </div>
              </div>
            </div>
