...
test.describe('First User', () => {
  ...
  await page.waitForURL('**/dashboard');
    ...
    await page.fill('[data-testid="register-senha"]', 'SenhaFort3!');
    await page.click('[data-testid="register-submit"]');
  await expect(page.locator('[data-testid="register-email-error"], [data-testid="register-error"]')).toContainText('E-mail já está em uso.');
  });
  ...
  await page.waitForURL('**/dashboard');
    ...
    await page.fill('[data-testid="register-senha"]', 'SenhaFort3!');
    await page.click('[data-testid="register-submit"]');
  await expect(page.locator('[data-testid="register-cpfCnpj-error"], [data-testid="register-error"]')).toContainText('Já existe uma escola registrada com este CPF/CNPJ.');
  });
});
