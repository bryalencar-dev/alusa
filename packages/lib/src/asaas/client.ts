...
// ===== Customers / Subscriptions (simplificado) =====
...
export async function ensureCustomer(data: { name: string; cpfCnpj: string; email: string; existingId?: string | null }): Promise<string> {
  if (data.existingId) return data.existingId;
  const body: AsaasCustomerRequest = { name: data.name, cpfCnpj: data.cpfCnpj, email: data.email };
  const created = await asaasFetch<AsaasCustomerResponse>('/customers', { method: 'POST', body: JSON.stringify(body) });
  return created.id;
}

export async function createSubscription(params: { customerId: string; value: number; start: Date; reference: string }): Promise<AsaasSubscriptionResponse> {
  const body: AsaasSubscriptionRequest = {
    customer: params.customerId,
    value: params.value,
    billingType: 'UNDEFINED',
    cycle: 'MONTHLY',
    nextDueDate: params.start.toISOString().slice(0, 10),
    externalReference: params.reference
  };
  return asaasFetch<AsaasSubscriptionResponse>('/subscriptions', { method: 'POST', body: JSON.stringify(body) });
}
